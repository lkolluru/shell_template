...
function runArchive() {
   ...
   if [[ -e ${ARCHIVE_TABLES_PTO_MERKLEZIP_EXPORT_FILE} && -e ${CE_HDLP_S3ARCHIVE_SCRIPT} ]]; then
      cat ${ARCHIVE_TABLES_PTO_MERKLEZIP_EXPORT_FILE} |
         while read line; do
      ...
      fatal_log "one of the control files does not exist ${ARCHIVE_TABLES_PTO_MERKLEZIP_EXPORT_FILE},${CE_HDLP_S3ARCHIVE_SCRIPT} does not exist"
      exit 1
