...
function post_process_validations() {
    ...
    [ $# -ne 1 ] && error_log "${FUNCNAME[0]}: at least 1 arguments are required" && return 1
    ...
    [ $sql_import_return_code -ne 0 ] && error_log "${FUNCNAME[0]}: $(basename "${0}") failed to execute successfully" && return 1
    ...
    { ## parse the file and convert the csv files into array of 3 values for processing
        ...
        [ "${#validation_file_parser[@]}" -ne 1 ] && error_log "${FUNCNAME[0]}:control file needs to have only a single record" && return 1

        #IFS=',' read -ra validation_file_value_array <<<"$(echo "${validation_file_parser[0]}")"
        IFS=',' read -ra validation_file_value_array <<<"${validation_file_parser[0]}"
        ...
        info_log "${FUNCNAME[0]}:record counts from the db functional group $functional_group are Validation:$validation_fail_count and Compute:$compute_fail_count"

    }
    sql_control_file_return_code=$?
    [ $sql_control_file_return_code -ne 0 ] && error_log "${FUNCNAME[0]}: $(basename "${0}") failed to execute successfully" && return 1

    if [ "$validation_fail_count" -ne 0 ] && [ "$compute_fail_count" -eq 0 ]; then

        info_log "${FUNCNAME[0]}: $(basename "${0}") failed on query validations"
        return 2
    fi

    if [ "$compute_fail_count" -ne 0 ]; then

        info_log "${FUNCNAME[0]}: $(basename "${0}") failed on compute"
        return 1
    fi

    info_log "${FUNCNAME[0]}: $(basename "${0}") scoring validations completed"
