...
function transfer_scores_data_ftp() {
    ...
    CDA_ZIP_FILE_PATH="${OFLNSEL_SCORES_PREPZIP_FILES_DIR}"/"${SCORES_FILES_ROOT}T.zip"
    CDA_ZIP_FILE_NAME="${SCORES_FILES_ROOT}T.zip"
    MAINFRAME_ZIP_FILE_NAME="${SCORES_FILES_ROOT}XXX.zip"
    FTP_SERVER="${MAINFRAME_SERVER}"
    FTP_USER_NAME="${SSHPASS}"
    FTP_DIRECTORY="${MAINFRAME_DIR}"

    if ! test_path ${CDA_ZIP_FILE_PATH}; then
        return 1
    fi

    if ! rename_ftpitem "${CDA_ZIP_FILE_PATH}" "${FTP_DIRECTORY}" "${CDA_ZIP_FILE_NAME}" "${MAINFRAME_ZIP_FILE_NAME}" "${FTP_USER_NAME}" "${FTP_SERVER}"; then
...
function preprocess() {
    ...
    export OFLNSEL_MODEL_NAME="mod321"
...
function main() {
    ...
    if ! transfer_scores_data_ftp; then 
        exit 1
    fi 
