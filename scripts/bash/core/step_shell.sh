...
function gen_shell_jar_error() {
	...
	[ $# -ne 2 ] && error_log "${FUNCNAME[0]}: at least 3 arguments are required" && return 1

	JOB="$0"      # job name
	LASTLINE="$1" # line of error occurrence
	LASTERR="$2"  # error code
	error_log "${FUNCNAME[0]}:GEN JAR SHELL ERROR in ${JOB} : line ${LASTLINE} produced error code ${LASTERR}" 2>&1 | tee -a "${shell_jar_log_file}"
...
function prepare_log_file() {
	...
	if [ ! -d "$(append_character "${log_directory}" "/")" ]; then
		...
		fatal_log "${FUNCNAME[0]}:Log directory ${log_directory} does not exist."
	...
	if [ -e "${step_log_file}" ]; then
		...
		info_log "${FUNCNAME[0]}:$(basename "${0}")::APPLICATION STEP LOG:-------------------------------------------------" 2>&1 | tee -a "${step_log_file}"

		info_log "${FUNCNAME[0]}:Log file ${step_log_file} already exists. Starting new ${subject_area} process." 2>&1 | tee -a "${step_log_file}"
		...
		info_log "${FUNCNAME[0]}:Created new ${step_log_file}. Starting new ${subject_area} process." 2>&1 | tee -a "${step_log_file}"
...
function prepare_shell_jar_log_file() {
	...
	if [ -e "${shell_jar_log_file}" ]; then
		...
		info_log "${FUNCNAME[0]}:$(basename "${0}")::APPLICATION STEP JAR LOG:-------------------------------------------------" 2>&1 | tee -a "${shell_jar_log_file}"

		info_log "${FUNCNAME[0]}:Log file ${shell_jar_log_file} already exists. Starting new ${subject_area} process." 2>&1 | tee -a "${shell_jar_log_file}"
		...
		info_log "${FUNCNAME[0]}:Created new ${shell_jar_log_file}. Starting new ${subject_area} process." 2>&1 | tee -a "${shell_jar_log_file}"
...
function clean_up() {
	...
	[ $return_code -eq 0 ] && info_log "${FUNCNAME[0]}: $(basename "${0}") completed successfully" 2>&1 | tee -a "${step_log_file}" && exit 0
	...
	[ $return_code -ne 0 ] && fatal_log "${FUNCNAME[0]}: $(basename "${0}") failed to complete and exiting with a consolidated exit code 1" 2>&1 | tee -a "${step_log_file}" && exit ${return_code}
...
function beehive() {
	...
	command -v /opt/mapr/hive/"${CDA_HIVE_VERSION}"/bin/beeline >/dev/null 2>&1 || {
		error_log "${FUNCNAME[0]}: beeline not available"
		exit 254
	}
	...
	info_log "${FUNCNAME[0]}:query executing is ${hive_query}"
	#info_log "${FUNCNAME[0]}:opts for the query are ${opts}"

	/opt/mapr/hive/"${CDA_HIVE_VERSION}"/bin/beeline \
		--fastConnect=true \
		--silent=true \
		--outputformat=csv2 \
		-u "${jdbc_url}" \
	...
	[ $beehive_rc -ne 0 ] && error_log "${FUNCNAME[0]}:hive query execution failed with exit code" && return 1

	info_log "${FUNCNAME[0]}: Sqoop query :${hive_query} is complete" && return 0
...
function beehivecounts() {
	...
	if [ $beehive_rc -ne 0 ]; then
		error_log "${FUNCNAME[0]}:hive query execution failed with exit code" && return 1
	fi
	#info_log "${FUNCNAME[0]}: Sqoop query :${hive_query} is complete" && return 0
...
function beehivecsv() {
	...
	info_log "${FUNCNAME[0]}:query exporting is ${hive_query}"

	/opt/mapr/hive/"${CDA_HIVE_VERSION}"/bin/beeline --fastConnect=true --silent=true --showHeader=true --outputformat=csv2 --verbose=false -u "${jdbc_url}" -n "${service_account}" -w "${passowrd_file}" -e "${hive_query}" >>"${hive_csv_destination}"

	beehivecsv_rc=$?

	[ $beehivecsv_rc -ne 0 ] && error_log "${FUNCNAME[0]}:hive query execution failed with exit code" && return 1

	info_log "${FUNCNAME[0]}: beeline query csv export of:${hive_query} is complete" && return 0
...
function beehivecsvnoheader() {
	...
	/opt/mapr/hive/"${CDA_HIVE_VERSION}"/bin/beeline --fastConnect=true --silent=true --showHeader=false --outputformat=csv2 --verbose=false -u "${jdbc_url}" -n "${service_account}" -w "${passowrd_file}" -e "${hive_query}" >>"${hive_csv_destination}"

	beehivecsvnoheader_rc=$?

	[ $beehivecsvnoheader_rc -ne 0 ] && error_log "${FUNCNAME[0]}:hive query execution failed with exit code" && return 1
...
function scoopevalquery() {
	...
	command -v /opt/mapr/sqoop/sqoop-1.4.7/bin/scoop >/dev/null 2>&1 || {
		error_log "${FUNCNAME[0]}: scoop not available"
		exit 254
	}

	[ $# -ne 2 ] && error_log "${FUNCNAME[0]}: at least 2 arguments are required" && return 1

	local db_query="${1}"
	local sqoop_options_dir="${2}"

	info_log "${FUNCNAME[0]}:query executing is ${db_query}"
	...
	[ $scoopevalquery_rc -ne 0 ] && error_log "${FUNCNAME[0]}:scoop import failed with exit code" && return 1

	info_log "${FUNCNAME[0]}: Sqoop query :${db_query} is complete"
...
function scoopimportquery() {
	...
	[ $# -ne 3 ] && fatal_log "${FUNCNAME[0]}: at least one argument is required" && return 1

	local db_query="${1}"
	local scoop_res_dir="${2}"
	local sqoop_options_file="${3}"

	info_log "${FUNCNAME[0]}:Query getting executed from SQL SERVER is :${db_query}"
	info_log "${FUNCNAME[0]}:Options file mapreduce job is :${scoop_res_dir}"
...
function load_consolidated_step_exitcode() {
	...
	if [ ! -e "$process_status_file" ]; then
		...
		error_log "${FUNCNAME[0]}: No valid step status file is present" 2>&1 | tee -a "${step_log_file}"
		exit 254
