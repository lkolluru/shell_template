...
info_log "Cleanup function is called. exit code is ${final_return_code} " 2>&1 | tee -a ${log_file}
elif [ "${final_return_code}" == "7" ]; then
info_log "Cleanup function is called.exit code is ${final_return_code}" 2>&1 | tee -a ${log_file}
else
info_log "Cleanup function is called.exit code is ${final_return_code}" 2>&1 | tee -a ${log_file}
...
function inputfiledelay_warn() {
    ...
    if ([ ${curr_hour} -eq ${ALERT_HOUR_3} ] || [ ${curr_hour} -eq ${ALERT_HOUR_4} ]) && [ ${curr_min} -ge ${ALERT_MIN_FROM} ] && [ ${curr_min} -le ${ALERT_MIN_TO} ]; then
        ...
        if [ "$(ls -lt --time-style='+%Y-%m-%d' ${FILE_WATCH_EXPORT_COMPLETE_DIR}/oflnsel_export_trigger* | head -1 | awk '{ print $6 }')" != "$(date +"%Y-%m-%d")" ]; then
...
if [[ "${OFLNSEL_LAST_RUN_DATE}" < "$(date +"%Y-%m-%d")" ]]; then
    ...
    info_log "Result of sqoop query" 2>&1 | tee -a ${log_file}
