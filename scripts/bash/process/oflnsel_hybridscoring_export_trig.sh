...
OFLNSEL_LAST_RUN_DATE_FILE="${FILE_WATCH_HYBRID_SCORING_DIR}/oflnsel_export_last_run_date_file"

if [ -e ${OFLNSEL_LAST_RUN_DATE_FILE} ]; then

    TRIG_FILE="${HYBRIDSCORING_FILE_WATCH_IMPORT_READY_DIR}/oflnsel_export_trigger_$(date +"%Y%m%d").trig"
    OFLNSEL_LAST_RUN_DATE=$(date +%Y-%m-%d -d "$(cat $OFLNSEL_LAST_RUN_DATE_FILE | awk '{ print $1 }') + 6 day")
    TMP_SQOOP_RESULT="${log_directory}/tmp_sqoop_hybscoring_$(date +"%Y_%m_%d_%H_%M_%S")"
else
...
if [[ "${OFLNSEL_LAST_RUN_DATE}" < "$(date +"%Y-%m-%d")" ]]; then
    ...
    TRIG_QUERY="SELECT MAX(CAST(targetexchangedatetime AS date)) AS CURR_RUN_DATE,COUNT(*) AS SAS_COUNT  FROM 		[hdl_calc_engine_metastore].logginginfo.variableexchangelogrepositoryinfo WHERE exportid IN (28,30,33) and [targetexchangestatusid] = 12 and 	[targetexchangeoverallstatus] = 1 and [sourceexchangerecordcount] = [targetexchangerecordcount] and cast(targetexchangedatetime as date)  > '$OFLNSEL_LAST_RUN_DATE'";
