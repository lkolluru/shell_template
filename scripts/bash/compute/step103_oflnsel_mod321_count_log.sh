...
function load_control_table() {
    ...
    [ load_control_table_rc -ne 0 ] && error_log "$FUNCNAME: error initalizing the vars" && return 1
    ...
    if [ "${model_count}" != "" ] && [ "${model_count}" != "0" ]; then
        ...
        error_log "$FUNCNAME:Hive table: ${MODEL_COUNT_MOD321_TABLE}  is empty or count retrieval failed. Log entry not made.  please check"
