#!/bin/bash ARCHIVE_TABLES_HIST_EXPORT_FILE
...
function runArchive() {
   ...
   if ! test_path "${ARCHIVE_TABLES_HIST_EXPORT_FILE}"; then

      return 1

   fi

   mapfile -t scores_cloud_archive_items <"${ARCHIVE_TABLES_HIST_EXPORT_FILE}"
   ...
   for cloud_archive_item in "${scores_cloud_archive_items[@]}"; do
      ...
      if [ "${cmnt}" == "#" ]; then
         ...
         info_log "${FUNCNAME[0]}:Skipping the commented step ${cloud_archive_item}\n"
         ...
         info_log "${FUNCNAME[0]}:proceeding with archivin ${cloud_archive_item}"
